...
// Class to represent each image segment
class ImageSegment {
   // Store the column and row position of this segment
  ...
  calculateSegDrawProps() {
    this.drawWidth = Math.ceil(imgDrwPrps.width / numSegments) + 1;// Calculate the width and height of each segment based on the image's properties
    this.drawHeight = Math.ceil(imgDrwPrps.height / numSegments) + 1;
    this.drawXPos = this.rowPostion * (imgDrwPrps.width / numSegments) + imgDrwPrps.xOffset;// Calculate the initial draw position based on row, column, and image offset
    this.drawYPos = this.columnPosition * (imgDrwPrps.height / numSegments) + imgDrwPrps.yOffset;
  }

  draw() {
    // Calculate the current time in seconds and retrieve the amplitude level from the audio
    let time = millis() / 1000;
    let level = amplitude.getLevel();
    // Calculate dynamic offsets based on amplitude level and sine/cosine functions
    ...
    // Apply dynamic offsets to the segment’s draw position
